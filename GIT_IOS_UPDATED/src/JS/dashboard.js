...
const request = () => {
  ...
  $("#sendRequest").submit(function (e) {
    ...
    const data = {
      uid: localStorage.getItem("UID"),
      startdate: frmdata.get("startdate"),
      enddate: frmdata.get("enddate"),
      type: frmdata.get("leavetype"),
      reason: frmdata.get("reason"),
    };
    if (
      frmdata.get("startdate") < currentDate() ||
      frmdata.get("enddate") < currentDate()
    ) {
      showMessage("Warning", "Date should be present", "info");
      $("#submitrequest").prop("disabled", false);
      return;
    }
    if (frmdata.get("leavetype") == null) {
